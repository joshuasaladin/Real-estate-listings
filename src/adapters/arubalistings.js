// Adapter for https://arubalistings.com — itself an aggregator that carries
// listings from many Aruban brokers, so it is our highest-coverage source.
//
// The site could not be inspected from the development environment (it sits
// behind anti-bot protection for datacenter IPs), so this adapter parses
// defensively with two strategies:
//   1. JSON-LD (schema.org) blocks — many real estate sites embed structured
//      RealEstateListing/Product/Offer data; most reliable when present.
//   2. Heuristic card parsing — anchors that look like listing detail pages,
//      with price/beds/baths pulled from the surrounding card text.
// If the site blocks or changes markup, the sync run records the error and
// every other source continues unaffected.
...
const MAX_PAGINATION = 25; // safety cap per section

export async function fetchListings(source) {
  const all = new Map(); // external_id -> listing
  ...
  for (const { path, status } of PAGES) {
    for (let page = 1; page <= MAX_PAGINATION; page++) {
      ...
      try {
        ...
        if (page === 1) throw err; // first page failing = source failing
        break; // ran out of pagination
      }
      const found = parsePage(html, source, status);
      if (found.length === 0) break;
      const before = all.size;
      for (const l of found) all.set(l.external_id, l);
      if (all.size === before) break; // page repeated -> stop
    }
  }
  return [...all.values()];
}
...
function parsePage(html, source, status) {
  ...
  return dedupeById([
    ...parseJsonLd($, source, status),
    ...parseCards($, source, status),
  ]);
}
...
/** Strategy 2: heuristic listing-card parsing. */
function parseCards($, source, status) {
  const out = [];
  const seen = new Set();
  // Listing detail links on arubalistings look like /sale/<slug> or /rent/<slug>
  // or /listing/<slug>; accept common variants.
  $('a[href]').each((_, a) => {
    ...
    if (/\/(all|page|search)([/?]|$)/.test(path)) return; // index pages, not details
    if (seen.has(abs)) return;

    // The "card" is the nearest ancestor with enough text to hold details.
    const card = $(a).closest('article, li, .card, [class*="listing"], [class*="property"], div');
    const text = card.text().replace(/\s+/g, ' ').trim();
    if (!text) return;
    const price = parsePrice((text.match(/(?:US\$|Afl\.?|AWG|\$|ƒ)\s?[\d.,]{4,}/i) || [])[0]);
    const title =
    ...
    out.push({
      ...
      price_raw: price ? (text.match(/(?:US\$|Afl\.?|AWG|\$|ƒ)\s?[\d.,]{4,}/i) || [])[0] : null,
      status,
